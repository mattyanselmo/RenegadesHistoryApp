...
shinyUI(
  navbarPage('Renegades History',
                      ...
                      sidebarLayout(
                        sidebarPanel(
                          ...
                          selectInput('standings_week',
                                      label = "Through week:",
                                      choices = c("Season plot" = "Regular season", 
                                                  1:(max(dat$Week[dat$Season == max(dat$Season)])) - 1,
                                                  "Current" = max(dat$Week[dat$Season == max(dat$Season)]),
                                                  (max(dat$Week[dat$Season == max(dat$Season)]) + 1):22),
                                      selected = max(dat$Week[dat$Season == max(dat$Season)])),
             ...
             tabPanel(title = 'Playoffs',
                      sidebarLayout(
                        sidebarPanel(
                          width = 2,
                          
                          selectInput('playoffs_season',
                                      label = 'Select season:',
                                      choices = 2018,
                                      selected = 2018)
                        ),
                        ...
                        mainPanel(
                          ...
                          fluidRow(
                            h1("Consolation standings"),
                            tableOutput('consplayoffstable'),
                            br(),
                            br(),
                            h1("All-team playoff standings"),
                            tableOutput('allplayoffstable'))
                        )
                      )
             )
  )
